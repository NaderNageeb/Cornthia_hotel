...
<div class="wrapper">
    ...
    <section class="invoice">
      ...
      <div class="col-sm-2 invoice-col">
        ...
        <div class="col-sm-2 invoice-col">
          ...
          <address> 
		  <div class="form-group">
			 <input class="form-control date start " size="20" type="date"  Placeholder="Check In" name="start" id="from" data-date="" data-date-format="yyyy-mm-dd" data-link-field="any" data-link-format="yyyy-mm-dd">
		 </div>
          </address>
        </div>
        <!-- /.col -->
        ...
        <address>
        <div class="form-group"> 
		      <input class="form-control date end " size="20" type="date"   value=""  name="end" id="end" data-date="" data-date-format="yyyy-mm-dd" data-link-field="any" data-link-format="yyyy-mm-dd">
		  </div>
  ...
  echo $end = $_POST['end'];
  echo $status = $_POST['categ'];
  echo $room_name = $_POST['txtsearch'];

   if( $start =='' && $end =='' && $status =='' && $room_name =='' ){
    echo "<script>alert('Search by Status or room name , start date or end date ');window.location = 'index.php';</script>";
   }else{
    if($start !='' && $end !='' && $status !='' && $room_name !='')
 $sql ="SELECT * 
     ...
     AND DATE(`ARRIVAL`) >=  '".$start."'
		 AND DATE(`DEPARTURE`) <=  '".$end."' AND RS.STATUS='" .$status."'
     AND CONCAT( `ACCOMODATION`, ' ', `ROOM` , ' ' , `ROOMDESC`) LIKE '%" .$room_name ."%'";
    }
  ...
  $mydb->setQuery($sql);
    ...
    echo "<script>alert('No Data For Your Search');window.location = 'index.php';</script>";
